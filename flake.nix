  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        #
        # 1. vendorHash - для Go-модулей. Ломается когда меняется go.sum.
        # 2. gitHashes.flutter_js - для Flutter. Ломается когда меняется
        #    pubspec.lock и там есть пакет с source: git.
        #
        # Как чинить hash mismatch:
        # 1. Смотрите в error - там всегда есть строка `got: sha256-...`
        # 2. Копируете именно это значение (не `specified:`, это старое)
        # 3. Вставляете сюда на нужное место:
        #    - ошибка про ...-go-modules...             -> vendorHash
        #    - ошибка про ...-flutter_js-source... (или
        #      другое имя пакета)                       -> gitHashes."имя_пакета"
        # 4. Пересобираете. Если вылезла новая ошибка - то же самое для неё.
        #
        # Чтобы не угадывать по названию - можно сразу временно поставить
        # pkgs.lib.fakeHash вместо строки с хешем, собрать, и Nix сразу
        # выдаст реальный хеш взамен фейкового. Всё это видно в логах
        # воркфлоу, оттуда и брать хеши.
        # ==========================================================
